 ...
 */
class DemoPrefillTest {
    private val setups = mapOf("stored method" to DemoForms.storePaymentMethod(), "capture" to DemoForms.capture())
    ...
    fun `every field the demo asks a payer to type has a value`() {
        setups.forEach { (screen, setup) ->
            PayInMethodType.entries.forEach { method ->
                val values = DemoPrefill.valuesFor(method)
    ...
    fun `the prefill carries the method it was asked for`() {
        PayInMethodType.entries.forEach { method ->
            assertEquals(method, DemoPrefill.valuesFor(method).method)
        }
    }
    ...
    fun `the card expiry is still ahead`() {
        ...
        val expiry = DemoPrefill.valuesFor(PayInMethodType.Card)[PayInField.CardExpiration]
        val month = YearMonth.parse(expiry, DateTimeFormatter.ofPattern("MM/uu"))

        assertTrue(
            "the prefilled expiry $expiry has passed: give DemoPrefill a later month",
