 ...
 * all and is the recommended general-purpose choice, but it lives on `android.os.SystemClock`, and consumers
 * run this in JVM unit tests as well, where that class does not exist.
 *
 * The conversion matches `PayabliService.elapsedMillis`, so a test measures a duration the way the code under
 * test does.
 */
public fun elapsedMillisSince(startedAtNanos: Long): Long =
