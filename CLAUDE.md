...
- **`src/sharedTest/java` is compiled into both `test` and `androidTest`**, wired in `core/build.gradle.kts`,
  ...
  suit a test layout. Do not re-propose it. A separate cross-module fixtures module is a different thing
...
- **CI runs no instrumented test.** All jobs are `ubuntu-latest` with no emulator, so `connectedAndroidTest`
  ...
  request red. A manual and nightly emulator job is planned, deliberately not a required per-PR check.
