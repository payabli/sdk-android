...
- A capability module depends on `:core` and **never** on a sibling capability. The umbrella re-exports every capability module, `:taptopay` included, so taking `sdk-android` takes the card reader.
...
- `minSdk` is **per module, not global**: `:taptopay` is **30**, required by the card reader dependency, and an app linking it must also be 30 or higher. `:payabli-android` is **30** because it re-exports `:taptopay`. The card-not-present modules are **23** and stay there. `:example` is **30**, and publishes nothing. It links `:taptopay`, so it takes that floor; 24 is the floor it would otherwise have, because its debug build reaches the local token server over cleartext and a network security config, which is the only way to permit that for two addresses instead of for everything, is ignored below 24.
- **`:taptopay` is 64-bit only, and nothing declares it.** The card reader pulls `magiccube`, whose AAR carries exactly one native library — `jni/arm64-v8a/libmc3.so`, with no 32-bit build. What that means for an `armeabi-v7a`-only handset depends on the rest of the APK, and both outcomes have been measured. An APK whose only native libraries come from `:taptopay` does not install at all: the install fails with `INSTALL_FAILED_NO_MATCHING_ABIS` before any code runs, which names the ABI but not the dependency that caused it, measured on an SM-A136U1 (`ro.product.cpu.abilist=armeabi-v7a,armeabi`). An APK that carries a 32-bit library from anything else installs and runs, and only card-present fails: `:example` is in that position, because `androidx.graphics.path` ships `lib/armeabi-v7a/`, which is readable from the built APK. So the install outcome is a property of the whole artifact rather than of this module, and the reader is unavailable either way. This is a vendor constraint like the API 30 floor, it is not expressed in any manifest or Gradle setting, and it means such a device cannot run this module's instrumented tests at all — a stated skip on the bench, not a failure to chase.
